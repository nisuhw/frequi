...
const handleImportSubmit = async () => {
  ...
  for (const bot of bots) {
    try {
      ...
      failedBots.push({ name: bot.botName || 'Unknown', error: e?.message || String(e) });
    }
  }
...
const importPlaceholder = computed(() => {
  return `JSON array in plain text (i.e [{"botName":"Bot Name","url":"${auth.value.url}","username":"Freqtrader","password":""}]) or base64 encoded (i.e W3siYm90TmFtZSI6IkJvdCBOYW1lIiwidXJsIjoiJHthdXRoLnVybH0iLCJ1c2VybmFtZSI6IkZyZXF0cmFkZXIiLCJwYXNzd29yZCI6IiJ9XQ==)`;
});
</script>
